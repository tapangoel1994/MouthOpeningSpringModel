...
%%Keeping all othe parameters fixed, this script varies the crossover
%%length scale (l0), the strength of the active force (f0) and the arrival
%%rate of the active force (1/tau). For each set of parameter values (l0, f0,tau),
...
l0 = [10 30 Inf]; %cross over length scale in microns
...
foldername = ['IndepedentInnerRing' num2str(N) 'Node' num2str(R) 'Ring'];
mkdir(foldername);
    ...
    for j = 1:1:length(tau) 
        for k = 1:1:length(gamma0)
            for ensamble = 1:1:5    
                ...
                poissonforce(2:end,:,:) = 0; %Only innermost ring has forces. everything else is zero
