...
function drdt = func2D(t0,r0,mu,k_rad,gamma_rad,k_azim,gamma_azim, b_azim, poissonforce,dt,n,radial_dist)

 
 drdt = zeros(size(r0));  %%Initialize the derivative vector. Last row is derivate for the fixed boundary which remains zero

 drdt(1:end-1,:,:) = (1/mu)*TotalForce(r0,k_rad,gamma_rad,k_azim,gamma_azim, b_azim, poissonforce,t0,dt,n,radial_dist);
