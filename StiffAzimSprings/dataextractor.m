...
%%largest mouth area.
%%Outputs [Force Tau Gamma k_azim Iter MinArea MaxArea MeanRadius StdRadius MajorRadius MinorRadius MajorAxisOrientation OpeningTime flag COMx COMy] as a row vector;
...
Key = 'azim_strength';
Index = strfind(Str, Key);
k_azim = sscanf(Str(Index(1) + length(Key):end), '%g', 1);
Key   = ',i';
Index = strfind(Str, Key);
...
if(length(yData)>50) %there are atleast 50 datapoints on the curve.
        ...
        if(MaxArea > MaxAreaUpperLimit)
            flag = -1;
            SimResults = [Force Tau Gamma k_azim Iter MinArea MaxArea MeanRadius StdRadius nan nan nan nan flag COMx COMy];
            %return ;
        end
        ...
        if(OpeningTime < OpeningTimeLowerLimit)
            ...
            SimResults = [Force Tau Gamma k_azim Iter MinArea MaxArea MeanRadius StdRadius nan nan nan OpeningTime flag COMx COMy];
            %return ;
        end
        ...
        SimResults = [Force Tau Gamma k_azim Iter MinArea MaxArea MeanRadius StdRadius MajorRadius MinorRadius MajorAxisOrientation OpeningTime flag COMx COMy];
else
       SimResults = [Force Tau Gamma k_azim Iter nan nan nan nan nan nan nan nan -20 nan nan];
end
