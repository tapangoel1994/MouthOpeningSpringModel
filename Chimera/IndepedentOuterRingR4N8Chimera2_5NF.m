...
%%processing
%% Nodes (r,N=2),(r,N=3),(r,N=4) and (r,N=5) are treated as being nerve free (so no external forcing on these nodes)
...
foldername = ['IndepedentOuterRing' num2str(N) 'Node' num2str(R) 'RingChimera'];
mkdir(foldername);
...
parfor ii = 1:1:length(forcing_strength)%[.001 .005 .01 .05 .1 .5 1 5 10]
    for j = 1:1:length(tau) %[.01 .05 .1 .5 1 5 10]; //average time between contractile forcing
        for k = 1:1:length(gamma0)
            ...
            %%Further since this is chimera, nodes 2-5 on the outerring
            %%dont get force
            poissonforce(1:end-1,:,:) = 0;
            poissonforce(end,2:5,:,:) = 0;
