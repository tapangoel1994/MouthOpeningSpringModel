...
function force  = TotalForce(r0,k_rad,gamma_rad,k_azim,gamma_azim,b_azim,...
                    poissonforce,t0,dt,n,radial_dist, microasp_force)
...
force = NaN(R+1,N,2);  %Initializes all forces to be NaN so that an error pops up if there is some error in the computation
...
%% force on outermost ring - microaspiration force directed radially out.
force(R+1,:,:) = 0;

force(R+1,2,:) = microasp_force*dirvec(squeeze(r0(R+1,2,:)));
force(R+1,6,:) = microasp_force*dirvec(squeeze(r0(R+1,6,:)));
end
